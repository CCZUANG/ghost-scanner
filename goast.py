...
def plot_interactive_chart(symbol):
    ...
    layout_common = dict(
        xaxis_rangeslider_visible=False, height=600,  margin=dict(l=10, r=10, t=50, b=50), 
    ...
    with tab3:
        try:
            ...
            df_4h['MA20'] = df_4h['Close'].rolling(window=20).mean(); df_4h['MA60'] = df_4h['Close'].rolling(window=60).mean()
...
def get_sp500_tickers():
    try: return [t.replace('.', '-') for t in pd.read_html(StringIO(requests.get("https://en.wikipedia.org/wiki/List_of_S%26P_500_companies", headers={"User-Agent": "Mozilla/5.0"}).text))[0]['Symbol'].tolist()]
    except: return []

@st.cache_data(ttl=3600)
def get_nasdaq100_tickers():
    try:
        dfs = pd.read_html(StringIO(requests.get("https://en.wikipedia.org/wiki/Nasdaq-100", headers={"User-Agent": "Mozilla/5.0"}).text))
        for df in dfs:
...
def get_ghost_metrics(symbol, vol_threshold):
    try:
        ...
        earnings_date = "未知"; cal = stock.calendar
        ...
        return {
            ...
            "Week Vol": round(log_ret.tail(5).std() * np.sqrt(252) * 100, 1) if len(log_ret) >= 5 else 0,
            ...
            "財報日": earnings_date, "題材搜尋": f"https://www.google.com/search?q={symbol}+美股+題材+風險+分析", "_sort_score": u_score
        }
    except: return None
